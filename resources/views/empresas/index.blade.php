...
@section('content')
    <h1 class="text-center text-info fw-bold"><i class="fa-solid fa-duotone fa-building"></i> {{ $head_title }}</h1>

    <button type="button" class="btn btn-success mb-3 btn-crear" data-bs-toggle="modal" data-bs-target="#modalCreateOrEdit">
        <i class="fa-solid fa-duotone fa-plus"></i> Crear empresa</button>

    <h2 class="text-info fw-bold">Lista de empresas</h2>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formCreateOrEdit">
                        <!-- input de idEmpresa en caso de editar -->
                        <input type="hidden" name="idEmpresa" value="0">

                        <div class="mb-3">
                            <label for="nombreEmpresa" class="form-label">Nombre de empresa <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="nombreEmpresa" name="nombreEmpresa" required>
                        </div>

                        <div class="mb-3">
                            <label for="bonoEmpresaPorcentaje" class="form-label">Bono (%) <span
                                    class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="bonoEmpresaPorcentaje" name="bonoEmpresaPorcentaje"
                                required>
                        </div>
                    </form>
                </div>
    ...
    <script>
        $(document).ready(function() {
            $("#dataTable").DataTable({
                processing: true,
                ajax: {
                    url: "{{ route('empresas.listar') }}", // Ruta de Laravel
                    type: "GET",
                    headers: {
                ...
                columns: [{
                    ...
                    <div class="btn-group" role="group">
                        <button type="button" class="btn btn-warning btn-sm btn-editar" 
                                data-id="${row.idEmpresa}" data-toggle="tooltip" title="Editar">
                            <i class="fa-duotone fa-solid fa-edit"></i>
                        </button>
                        <button type="button" class="btn btn-${row.estado == 1 ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                data-id="${row.idEmpresa}" data-estado="${row.estado}" data-nombre="${row.nombreEmpresa}" 
            ...
            $(document).on('click', '.btn-crear', function() {
                $('#formCreateOrEdit input[name="idEmpresa"]').val(0);
                $('#formCreateOrEdit input[name="nombreEmpresa"]').val('');
                $('#formCreateOrEdit input[name="bonoEmpresaPorcentaje"]').val('0');

                const titleElement = document.getElementById('modalCreateOrEdit_Title');
                titleElement.innerHTML = '<i class="fa-solid fa-duotone fa-plus"></i> CREAR EMPRESA';
            ...
            $(document).on('click', '.btn-editar', function() {
                ...
                $.get("{{ route('empresas.index') . '/' }}" + id, function(empresa) {
                    $('#formCreateOrEdit input[name="idEmpresa"]').val(empresa.data.idEmpresa);
                    $('#formCreateOrEdit input[name="nombreEmpresa"]').val(empresa.data.nombreEmpresa);
                    $('#formCreateOrEdit input[name="bonoEmpresaPorcentaje"]').val(empresa.data
                        .bonoEmpresaPorcentaje);
                    const titleElement = document.getElementById('modalCreateOrEdit_Title');
                    titleElement.innerHTML =
                        '<i class="fa-solid fa-duotone fa-edit"></i> EDITAR EMPRESA';
                    $('#modalCreateOrEdit').modal('show');
                });
            });
            ...
            $(document).on('click', '#btnGuardar', function() {
                const idEmpresa = $('#formCreateOrEdit input[name="idEmpresa"]').val();
                const url = idEmpresa == 0 ?
                    "{{ route('empresas.create') }}" // POST -> crear
                    :
                    "{{ route('empresas.index') . '/' }}" + idEmpresa; // PUT -> actualizar

                const type = idEmpresa == 0 ? 'POST' : 'PUT';
            ...
            $(document).on('click', '.btn-cambiar-estado', function() {
                ...
                Swal.fire({
                    title: `¡ATENCIÓN!`,
                    text: `¿Estás seguro de ${accion} la empresa ${nombre}?`,
                    icon: 'question',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function(xhr) {
                                console.error(xhr.responseText);
                                Swal.fire('Error', `No se pudo ${accion} la empresa`,
                                    'error');
                            }
                        });

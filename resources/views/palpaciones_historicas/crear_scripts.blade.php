<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const tableBody = document.querySelector('#table-palpaciones-historicas tbody');
        const fileInput = document.getElementById('excel-file');
        const btnGuardar = document.getElementById('btn-guardar');

        // ─── Helpers ────────────────────────────────────────────────────────────────

        function crearFila(carimbo = '', identificador = '', resultado = '', fecha = '') {
            ...
            tr.innerHTML = `
                ...
                <td><input type="text" class="form-control form-control-sm" placeholder="Resultado" value="${resultado}"></td>
        ...
        function reindexar() {
            document.querySelectorAll('#table-palpaciones-historicas tbody tr').forEach((tr, i) => {
                tr.querySelector('.fila-numero').textContent = i + 1;
            });
        }

        function limpiarErrores() {
            document.querySelectorAll('#table-palpaciones-historicas tbody tr').forEach(tr => {
        ...
        fileInput.addEventListener('change', function() {
            ...
            reader.onload = function(e) {
                ...
                rows.slice(start).forEach(row => {
                    ...
                    const [, carimbo, identificador, resultado, fechaRaw] = row;

                    const fecha = excelSerialToDate(fechaRaw);
                    tableBody.appendChild(crearFila(carimbo, identificador, resultado, fecha));
                });
        ...
        btnGuardar.addEventListener('click', function() {
            ...
            const filas = document.querySelectorAll('#table-palpaciones-historicas tbody tr');
            ...
            filas.forEach((tr, i) => {
                ...
                const resultado = inputs[2].value.trim();
                const fecha = inputs[3].value.trim();

                if (!carimbo || !identificador || !resultado || !fecha) {
                    tr.classList.add('table-warning');
                    tr.querySelector('.celda-observacion').textContent = 'Campos incompletos.';
                    hayVacios = true;
                }

                palpaciones_historicas.push({
                    carimbo,
                    identificador,
                    resultado,
                    fecha
            ...
            fetch('{{ route('palpaciones-historicas.create') }}', {
                    method: 'POST',
                    headers: {

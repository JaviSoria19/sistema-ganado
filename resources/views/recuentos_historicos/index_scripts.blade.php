<script>
    ...
    $(document).ready(function() {
        ...
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('recuentos-historicos.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
                ...
                },
                {
                    ...
                    <div class="btn-group" role="group">
                        <button type="button" class="btn btn-warning btn-sm btn-editar" 
                                data-id="${row.id_recuento_historico}" data-toggle="tooltip" title="Editar">
                            <i class="fa-duotone fa-solid fa-edit"></i>
                        </button>
                        <button type="button" class="btn btn-${row.estado == "activo" ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                data-id="${row.id_recuento_historico}" data-estado="${row.estado}" data-nombre="${row.bovino.identificador}" 
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $.get("{{ route('recuentos-historicos.index') . '/' }}" + id, function(recuento_historico) {
                $('#form-crear-o-editar input[name="id_recuento_historico"]').val(recuento_historico.data.id_recuento_historico);
                $('#form-crear-o-editar select[name="id_bovino"]').val(recuento_historico.data.id_bovino)
                    .trigger('change');
                $('#form-crear-o-editar input[name="peso"]').val(recuento_historico.data.peso);
                $('#form-crear-o-editar input[name="fecha"]').val(recuento_historico.data.fecha);

                const titleElement = document.getElementById('modal-formulario-titulo');
                titleElement.innerHTML =
                    `<i class="fa-solid fa-edit"></i> EDITAR RECUENTO HISTÓRICO`;
        ...
        $(document).on('click', '#btn-guardar', function() {
            const id_recuento_historico = $('#form-crear-o-editar input[name="id_recuento_historico"]').val();

            if (id_recuento_historico === 0) {
                swal.fire('Error', 'No se ha cargado un recuento histórico para editar.', 'error');
                return; // Evitar enviar el formulario si no se ha cargado un recuento histórico para editar
            }

            const url = id_recuento_historico == 0 ?
                "{{ route('recuentos-historicos.create') }}" // POST -> crear
                :
                "{{ route('recuentos-historicos.index') . '/' }}" +
                id_recuento_historico; // PUT -> actualizar

            const type = id_recuento_historico == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('recuentos-historicos.index') . '/' }}" + id,
                        type: "PATCH",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: {
                            id_recuento_historico: id
                        },
                        success: function(response) {
                        ...
                        error: function() {
                            Swal.fire('Error',
                                `No se pudo ${accion} el recuento histórico`,
                                'error');
                        }
                    });

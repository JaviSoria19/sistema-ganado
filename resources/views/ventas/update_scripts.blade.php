<script>
    // Inyectamos los datos de la venta provenientes de Laravel
    ...
    /* ═══════════════════════════════════════════════════════
       ESTADO GLOBAL (Inicializado con VENTA_ACTUAL)
    ═══════════════════════════════════════════════════════ */
    const STATE = {
        bovinos: VENTA_ACTUAL.bovinos.map(b => {
            const esFijo = parseFloat(b.pivot.precio_fijo) > 0;
            return {
                ...
                peso_actual: parseFloat(b.peso_actual || b.pivot.kg_peso_vivo),
                tipo_precio: esFijo ? 'fijo' : 'kg',
                precio_fijo: parseFloat(b.pivot.precio_fijo),
                precio_kg: parseFloat(b.pivot.precio_kg),
                destare: parseFloat(b.pivot.destare),
                rendimiento: parseFloat(b.pivot.rendimiento),
        ...
        pagos: VENTA_ACTUAL.pagos.map(p => ({
            id_pago: p.id_pago, // Importante para la edición (0 = nuevo, id = existente)
    ...
    async function cargarBovinos() {
        try {
            ...
            // Inyectar los bovinos que YA están en la venta para que puedan ser re-agregados si se quitan
    ...
    /* ═══════════════════════════════════════════════════════
       MODAL CLIENTE (Lógica compartida)
    ═══════════════════════════════════════════════════════ */
    ...
    document.getElementById('btn-guardar-cliente').addEventListener('click', async () => {
        ...
        const isEdit = editId !== '';
        const url = isEdit ? `{{ route('clientes.update', ':id') }}`.replace(':id', editId) :
    ...
    function renderPagos() {
        ...
        STATE.pagos.forEach((pago, idx) => {
            ...
            <div class="col-sm-4">
                <input type="date" class="form-control form-control-sm pago-fecha" data-idx="${idx}"
                    value="${pago.fecha_pago ? pago.fecha_pago.split(' ')[0] : ''}">
            </div>
        ...
        cont.querySelectorAll('.pago-btn-quitar').forEach(btn => {
            btn.addEventListener('click', () => {
                return showAlert('Funcionalidad de eliminación de pagos no permitida en edición. Para eliminar un pago, edítalo y pon monto en 0.', 'warning');
    ...
    document.getElementById('btn-guardar-venta').addEventListener('click', async () => {
        ...
        if (!idCliente) return showAlert('Selecciona un cliente para continuar.');
        if (!fechaVenta) return showAlert('Indica la fecha de venta.');
        if (!STATE.bovinos.length) return showAlert('Agrega al menos un bovino a la venta.');

        for (const b of STATE.bovinos) {
            if (b.tipo_precio === 'fijo' && b.precio_fijo <= 0) return showAlert(
                `El bovino <b>${b.identificador}</b> tiene precio fijo en 0. Ingresa un valor válido.`
                );
            if (b.tipo_precio === 'kg' && b.precio_kg <= 0) return showAlert(
    ...
    document.getElementById('btn-confirmar-eliminar').addEventListener('click', async () => {
        ...
        if (!motivo) {
            return showAlert('Debe proporcionar un motivo de eliminación.', 'warning');
        }
        ...
        try {
            ...
            const data = await r.json();
            if (data.success) {
                showAlert(data.message ?? 'Venta eliminada.', 'success');

<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const tableBody = document.querySelector('#table-capacidades-historicas tbody');
        const fileInput = document.getElementById('excel-file');
        const btnGuardar = document.getElementById('btn-guardar');

        // ─── Helpers ────────────────────────────────────────────────────────────────

        function crearFila(nombre = '', capacidad_carga = '', fecha = '') {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td class="text-center align-middle fila-numero"></td>
                <td><input type="text"   class="form-control form-control-sm" placeholder="Nombre del potrero" value="${nombre}"></td>
                <td><input type="number" step="0.01" class="form-control form-control-sm" placeholder="Capacidad de carga (ua)" value="${capacidad_carga}"></td>
        ...
        function reindexar() {
            document.querySelectorAll('#table-capacidades-historicas tbody tr').forEach((tr, i) => {
                tr.querySelector('.fila-numero').textContent = i + 1;
            });
        }

        function limpiarErrores() {
            document.querySelectorAll('#table-capacidades-historicas tbody tr').forEach(tr => {
        ...
        fileInput.addEventListener('change', function() {
            ...
            reader.onload = function(e) {
                ...
                rows.slice(start).forEach(row => {
                    ...
                    const [, nombre, capacidad_carga, fechaRaw] = row;

                    const fecha = excelSerialToDate(fechaRaw);
                    tableBody.appendChild(crearFila(nombre, capacidad_carga, fecha));
                });
        ...
        btnGuardar.addEventListener('click', function() {
            ...
            const filas = document.querySelectorAll('#table-capacidades-historicas tbody tr');
            ...
            const capacidades_historicas = [];
            let hayVacios = false;

            filas.forEach((tr, i) => {
                const inputs = tr.querySelectorAll('input');
                const nombre = inputs[0].value.trim();
                const capacidad_carga = inputs[1].value.trim();
                const fecha = inputs[2].value.trim();

                if (!nombre || !capacidad_carga || !fecha) {
                    tr.classList.add('table-warning');
                    tr.querySelector('.celda-observacion').textContent = 'Campos incompletos.';
                    hayVacios = true;
                }

                capacidades_historicas.push({
                    nombre,
                    capacidad_carga,
                    fecha
                });
            ...
            fetch('{{ route('capacidades-historicas.create') }}', {
                    method: 'POST',
                    headers: {
